...
def _launch(goal_id: int, worker: dict, site_dir: Path) -> WorkerHandle:
    ...
    """
    try:
        container = catalog.launch_container(worker, goal_id, site_dir)
        ...
        # )
        handle = WorkerHandle(container, time.monotonic())
        return handle
    except Exception as e:
        logger.error(f"Exception {e} occurred while launching worker")
        return None

def _terminate(container: Container) -> None:
    """Stop a container"""
    try:
        ...
        logger.debug(f"{container.name} logs - {logs}")
        container.remove(force=True)
    except Exception as e:
        logger.error(f"Exception while terminating container - {e}")
        pass

def _terminate1(proc: subprocess.Popen) -> None:
...
class Team:
    ...
    def __init__(self, language: str, workers: list[dict], site_dir: Path, board_path: Path) -> None:
        ...
        self.pending: list[WorkerHandle] = []
...
def make_tools(team: Team) -> list:
    ...
    def launch_worker(framework: str, objective: str) -> str:
        ...
        team.pending.append(_launch(goal_id, worker, team.site_dir))
        logger.debug(f"Launched {worker['name']} to invent a {team.language} game teaching {objective} (folder {slug}/).")
            ...
            while True:
                ...
                for worker in handles:
                    ...
                    if worker.container.status == "running":
                        running = True
                        if (time.monotonic() - worker.started) > WORKER_TIMEOUT:
                            logger.warn(
                                f"Stopping {worker.container.name}"
                            )
                            worker.container.stop(timeout=5)
                if not running:
                    break
                await asyncio.sleep(2)
            live.update(live_board.render(team.registry))
            for worker in handles:
                _terminate(worker.container)
    ...
    print("Make tools completed successfully")
...
def _build_agent(team: Team) -> LlmAgent:
    ...
    try:
        agent = LlmAgent(name="orchestrator", model=config.ORCHESTRATOR_MODEL, instruction=instruction, tools=make_tools(team))
        return agent
    except Exception as e:
...
async def _run(team: Team) -> None:
    ...
    try:
        ...
        print(f"\n  NOTE: the orchestrator stopped early after an error ({type(exc)}); wrapping up with what is built.")
    finally:
        try:
            await runner.close()
        except Exception:
