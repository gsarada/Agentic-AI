...
def save_file(name, file, is_profile):
    try:
        ...
        if ext == 'pdf':
            ...
            final_text = Path(file).read_text()
...
def load_file(name, filename):
    ...
    try:
        ...
        print(f"Exception while loading file {file_path}")
    return final_text
...
def process_candidate_data(name: str, linkedin_file, exp_file, jd_file, state):
    ...
    try:
        ...
        if linkedin_file.get("files"):
            profile_text = save_file(name, linkedin_file["files"][0], True)
        ...
        if exp_file.get("files"):
            exp_summary = save_file(name, exp_file["files"][0], False)
            ...
            exp_summary = load_file(name, "experience_chunks.json")
